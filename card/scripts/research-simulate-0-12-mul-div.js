 ...
 *
 * Simplified policy simulation (same shape as plus-minus research script).
 */
...
const OUT_JSON = path.join(__dirname, '..', 'docs', 'research', 'research-0-12-mul-div.json');
...
function generateDeck() {
  ...
  for (const op of ['x', '÷']) {
    for (let i = 0; i < (op === '÷' ? 3 : 4); i++) cards.push({ id: makeId(), type: 'operation', operation: op });
  }
